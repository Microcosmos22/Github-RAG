...
Finding the correct context / code snippet $snippet_i$ from the user query $query$ is the main task of a RAG, and is done in embedding space. That is, we use the encoder to find the vectors $\eta_s^i$ and $\eta_q$ and search the snippets using either FlatIndex or InverseFileIndex to find the best `top_k` matching snippets: $Min_i^($\eta_s^i$, $\eta_q$). We usually search for the best 10 matching snippets {\{i\}}.
